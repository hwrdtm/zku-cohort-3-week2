...
describe("Custom Tests", function () {
  ...
  it("[assignment] iii. Alice deposit 0.13 ETH in L1 -> Alice send Bob 0.06 ETH in L2 -> Bob withdraw all ETH in L2 -> Alice withdraw all ETH in L1", async () => {
    // [assignment] complete code here
    ...
    // Alice sends Bob 0.06 ETH in L2
    ...
    // Bob withdraw ALL ETH in L2 from the shielded pool
    ...
    // Assert Utxo amount is 0 (since withdrawing all funds)
    expect(bobWithdrawUtxo.amount).to.be.equal(0);
    ...
    // Alice withdraw ALL ETH in L1
    const aliceWithdrawAmount = utils.parseEther("0.07");
    const aliceEthAddress = "0x0089d53F703f7E0843953D48133f74cE247184c2";
    ...
    // Assert Utxo amount is 0 (since withdrawing all funds)
    expect(aliceWithdrawUtxo.amount).to.be.equal(0);
    ...
    // All balance assertions

    // Assert Bob L2 balance
    const bobBalance = await token.balanceOf(bobEthAddress);
    expect(bobBalance).to.be.equal(bobWithdrawAmount);

    // Assert Alice L2 balance
